...
export default function SantasCenas() {
  ...
  const loadData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setData(formatted.filter((s: Servicio) => s.tipo === 'santa_cena'));
      }
  ...
  const handleOpenEdit = (item: Servicio) => {
    ...
    // Find IDs based on names (fallback if API doesn't return IDs)
  ...
  const columns = [
    { header: 'Fecha Realizada', accessor: 'fecha' as keyof Servicio },
  ...
  return (
    <div className="space-y-6">
      <DataTable
        title="Registro General de Santas Cenas"
        description="Bitácora administrativa de todas las santas cenas registradas a nivel nacional."
        data={data}
        columns={columns}
