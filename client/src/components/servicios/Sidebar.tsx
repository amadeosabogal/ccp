...
export default function Sidebar({ onLogout, isOpen = false, setIsOpen }: SidebarProps) {
  const location = useLocation();

  const navigation = [
    { name: 'Dashboard', href: '/servicios/admin', icon: LayoutDashboard },
    { name: 'Ancianos', href: '/servicios/admin/ancianos', icon: Users },
    { name: 'Salas de Oración', href: '/servicios/admin/salas', icon: Church },
    { name: 'Bautizos', href: '/servicios/admin/bautizos', icon: Droplets },
    { name: 'Santas Cenas', href: '/servicios/admin/santas-cenas', icon: Wine },
  ];
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-6">
          <nav className="space-y-1">
            {navigation.map((item) => {
              const isActive = location.pathname === item.href || (item.href !== '/servicios/admin' && location.pathname.startsWith(item.href));
              return (
                <Link
                  key={item.name}
                  to={item.href}
